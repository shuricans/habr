...
export class DesignPageComponent implements OnInit, OnDestroy {
  ...
  private readonly DESING_TOPIC = 'Дизайн';

  constructor(private postService: PostService,
              private dataService: DataService) {
    this.postFilter = new PostFilter();
    this.postFilter.topic = this.DESING_TOPIC;
  }

  ngOnInit(): void {
    this.pageFilter = this.dataService.getDesignPageFilter();
    this.getPage(this.pageFilter.page);
  }

  ngOnDestroy() {
    this.dataService.setDesignPageFilter(this.pageFilter);
  }
